...
class HReplayBuffer(ReplayBuffer):
    ...
    _reserved_keys = (
        ...
        "policy", "is_start", "rew_prev", "env_step", "h_rew", "h_act", 'c_distribution'
    )
    _input_keys = (
    ...
    def from_data(
            ...
            obs_next: h5py.Dataset, env_step, h_rew, h_act, c_distribution
    ) -> "ReplayBuffer":
        size = len(obs)
        assert all(len(dset) == size for dset in [obs, act, rew, terminated,
                                                  truncated, done, obs_next,
                                                env_step, h_rew, h_act, c_distribution]), \
    ...
    def __getitem__(self, index: Union[slice, int, List[int], np.ndarray]) -> Batch:
        ...
        batch_dict = {
            ...
            "c_distribution": self.get(indices, "c_distribution", Batch()),
        }
