...
class Collector(object):
    ...
    def collect(
        ...
        cnt_loop = 0
        c_distribution = np.array(self.env.category_distribution)[np.array(ready_env_ids)]
        while True:
           # assert len(self.data) == len(ready_env_ids)
            ...
            if random:
                ...
                if self.is_hrl:
                    ...
                    if indices is not None:
                       ...
                       # c_distribution = c_distribution[ready_env_ids] if n_episode else c_distribution
                    if no_grad:
                        with torch.no_grad():  # faster than retain_grad version
                            ...
                            result = self.policy(self.data, self.buffer, indices=indices, is_obs=True, state=last_state, 
                                                 ...
                                                 use_batch_in_statetracker=True, c_distribution=c_distribution)
                    else:
                        result = self.policy(self.data, self.buffer, 
                                             indices=indices, is_obs=True, state=last_state,
                                             remove_recommended_ids=self.remove_recommended_ids, is_train=is_train,
            ...
            step_arr.append(np.copy(self.env_step))
            self.env_step += 1
            ...
            if self.is_hrl:
                obs_next, rew, h_rew, c_dis, terminated, truncated, info = self.env.step(action_remap, ready_env_ids)
                c_distribution = c_dis[~terminated]
                self.data.update(h_rew=h_rew, c_distribution=c_dis)
            else:
                obs_next, rew, _, c_dis, terminated, truncated, info = self.env.step(action_remap, ready_env_ids)
            category_dis_arr.append(c_dis)
            rew_arr.append(rew)
        ...
        if current_epoch is not None:
            step_arr = np.concatenate(step_arr, axis=0)
            category_dis_arr = np.concatenate(category_dis_arr, axis=0)
            rew_arr = np.concatenate(rew_arr, axis=0)
            analyse_res = {'env_step': step_arr, 'c_dis': category_dis_arr, 'rew': rew_arr}
            if self.is_hrl:
                h_act_arr = np.concatenate(h_act_arr, axis=0)
                analyse_res['h_act'] = h_act_arr
            return res, analyse_res
        else:
            return res