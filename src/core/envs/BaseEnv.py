...
class BaseEnv(ABC, gym.Env):
    ...
    def step(self, action):
        self.action = action
        t = self.total_turn
        terminated, category_distribution = self._determine_whether_to_leave(t, action)
        if t + self.bored_punishment >= (self.max_turn - 1):
            terminated = True
        ...
        return self.state, reward, 0, category_distribution, terminated, truncated, {'cum_reward': self.cum_reward}
