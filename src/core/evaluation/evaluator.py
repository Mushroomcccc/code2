...
class Evaluator_Feat_Entropy():
    def __init__(self, test_collector_set, df_item_val, need_transform, item_feat_domination, lbe_item, top_rate, draw_bar=False, env=None):
    ...
    def on_epoch_end(self, epoch, results=None, **kwargs):
        ...
        def get_actions_feat_e(buffer, indices):
            ...
            category_distribution = np.array(self.env.category_distribution.shape[0] * [0.])
            for x in all_acts_origin.reshape(-1):
                c = self.env.list_feat[x]
                category_distribution[c] += 1
            p = category_distribution
            p = p / np.sum(p) 
            eps = 1e-10 
            entropy = -np.sum(p * np.log(p + eps))
            res = {'Category_entropy': entropy}
            return res
        ...
        for name, collector in self.collector_dict.items():
            ...
            res = get_actions_feat_e(buffer, indices)
